...
from dictionary.views.images import (
    ImageList,
    ImageUpload,
    ImageDetailProduction,
    ImageDetailDevelopment,
)
from dictionary.views.list import (
    ActivityList,
...
urlpatterns_user = [
    ...
    path(
        "messages/archive/", ConversationArchiveList.as_view(), name="messages-archive"
    ),
    path("messages/<slug:slug>/", Chat.as_view(), name="conversation"),
    path(
        "messages/archive/<slug:slug>/",
        ChatArchive.as_view(),
        name="conversation-archive",
    ),
    path("author/<slug:slug>/", UserProfile.as_view(), name="user-profile"),
    path(
        "author/<slug:slug>/<slug:tab>/",
        UserProfile.as_view(),
