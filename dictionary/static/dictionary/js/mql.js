...
const scrollTopButton = template(
  `<a role="button" tabindex="0" class="bg-light rounded border-0 w-100 p-2 d-none scrolltop text-center" style="display: block">${gettext(
    "scroll to top"
  )}</a>`
)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (requestMessages.getAttribute("data-has-messages") === "true") {
    ...
    requestMessages.childNodes.forEach(message => {
      ...
      notify(
        message.getAttribute("data-message"),
        message.getAttribute("data-level"),
        delay,
        isPersistent
      )
      delay += 1000
