...
function entryAction(type, pk, redirect = false) {
  return gqlc({
    query: `mutation{entry{${type}(pk:"${pk}"){feedback ${
      redirect ? "redirect" : ""
    }}}}`,
  })
}
...
Handler("a.favorite[role='button']", "click", function () {
  ...
  gqlc({ query: `mutation{entry{favorite(pk:"${pk}"){feedback count}}}` }).then(
    response => {
      ...
      this.parentNode
        .querySelector("span.favorites-list")
        .setAttribute("data-loaded", "false")
    }
  )
})
...
Handler("a.fav-count[role='button']", "click", function () {
  ...
  gqlc({ query: `{entry{favoriters(pk:${pk}){username slug isNovice}}}` }).then(
    response => {
      ...
      if (!allUsers.length) {
        favoritesList.innerHTML = `<span class='p-2'>${gettext(
      ...
      if (novices.length > 0) {
        ...
        const noviceToggle = template(
          `<a role="button" tabindex="0">${noviceString}</a>`
        )
...
Handler("a.twitter[role='button'], a.facebook[role='button']", "click", function () {
  ...
  const entry = this.closest(".entry-footer")
    .querySelector(".meta .permalink")
    .getAttribute("href")
...
Handler(".comment-vote .vote", "click", function () {
  ...
  gqlc({
    query:
      "mutation($pk:ID!,$action:String!){entry{votecomment(pk:$pk,action:$action){count}}}",
    variables: { pk, action },
  }).then(response => {
    if (response.errors) {
    ...
    this.classList.toggle("active")
    this.parentNode.querySelector(".rating").textContent =
      response.data.entry.votecomment.count
  })
})

Handler(".entry-actions", "click", function (event) {
  const [action] = ["message", "pin", "block", "delete", "copy"].filter(action =>
  ...
  switch (action) {
    ...
    case "delete": {
      if (confirm(gettext("are you sure to delete?"))) {
        ...
        entryAction("delete", entry.getAttribute("data-id"), redirect).then(
          response => {
...
Handler(".entry-full a.action[role='button']", "click", function () {
  ...
  const topicTitle = encodeURIComponent(
    entry.closest("[data-topic]").getAttribute("data-topic")
  )
  const actions = this.parentNode.querySelector(".entry-actions")
  const pinLabel =
    entryID === one("body").getAttribute("data-pin")
  ...
  if (userIsAuthenticated) {
    if (entry.classList.contains("commentable")) {
      actions.innerHTML += `<a target="_blank" href="/entry/${entryID}/comment/" class="dropdown-item">${gettext(
        "comment"
      )}</a>`
    }
    if (entry.classList.contains("owner")) {
      actions.innerHTML += `<a role="button" tabindex="0" class="dropdown-item pin">${pinLabel}</a>`
      actions.innerHTML += `<a role="button" tabindex="0" class="dropdown-item delete">${gettext(
        "delete"
      )}</a>`
      actions.innerHTML += `<a href="/entry/update/${entryID}/" class="dropdown-item">${gettext(
        "edit"
      )}</a>`
    } else {
      if (!entry.classList.contains("private")) {
        actions.innerHTML += `<a role="button" tabindex="0" class="dropdown-item message">${gettext(
          "message"
        )}</a>`
        actions.innerHTML += `<a role="button" tabindex="0" class="dropdown-item block">${gettext(
          "block"
        )}</a>`
      }
    }
  ...
  if (isTouchDevice) {
    actions.innerHTML += `<a role="button" tabindex="0" class="dropdown-item copy">${gettext(
      "copy link"
    )}</a>`
  }

  actions.innerHTML += `<a class="dropdown-item" href="/contact/?referrer_entry=${entryID}&referrer_topic=${topicTitle}">${gettext(
    "report"
  )}</a>`
...
Handle(".delete-entry-redirect", "click", function () {
  if (confirm(gettext("are you sure to delete?"))) {
    entryAction("delete", this.getAttribute("data-target-entry"), true).then(
      response => {
  ...
  many("article.entry p").forEach(el => {
    if (overflown(el)) {
      const readMore = template(
        `<div role="button" tabindex="0" class="read_more">${gettext(
          "continue reading"
        )}</div>`
      )
...
function draftEntry(content, pk = null, title = null) {
  return gqlc({
    query:
      "mutation($content:String!,$pk:ID,$title:String){entry{edit(content:$content,pk:$pk,title:$title){pk,content,feedback}}}",
    ...
    if (response.errors) {
      ...
      if (!btn.hasAttribute("data-pk")) {
        ...
        btn.after(
          template(
            `<button type="button" class="btn btn-django-link fs-90 ml-3 draft-del">${gettext(
              "delete"
            )}</button>`
          )
        )
    ...
    draftEntry(content, null, title).then(response => {
      if (response) {
        one(".user-content").prepend(
          template(
            `<section class="pw-area"><h2 class="h5 text-muted">${gettext(
              "preview"
            )}</h2><p class="text-formatted pw-text">${
              response.data.entry.edit.content
            }</p></section>`
          )
        )
        const pk = response.data.entry.edit.pk
        this.setAttribute("data-pk", pk)
        one("#content-form").prepend(
          template(`<input type="hidden" name="pub_draft_pk" value="${pk}" />`)
        )
      }
    })
  }
