...
const showImageErrorMessage = () => {
  notify(
    gettext("image could not be displayed. it might have been deleted."),
    "error",
    2200
  )
}
  ...
  if (self.matches(".entry a[data-img], .text-formatted a[data-img]")) {
    ...
    if (!self.hasAttribute("data-loaded")) {
      ...
      const image = template(
        `<img src="${url}" alt="${gettext(
          "image"
        )}" class="img-thumbnail img-fluid" draggable="false">`
      )
      const expander = template(
        `<a rel="ugc nofollow noopener" title="${gettext(
          "open full image in new tab"
        )}" href="${url}" target="_blank" class="ml-3 position-relative" style="top: 2px;"></a>`
      )
