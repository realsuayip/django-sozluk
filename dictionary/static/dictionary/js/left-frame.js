  ...
  renderTabs(tabData) {
    ...
    if (tabData) {
      ...
      tabData.available.forEach(tab => {
        tabHolder.innerHTML += `<li class="nav-item"><a role="button" tabindex="0" data-lf-slug="${
          this.slug
        }" data-tab="${tab.name}" class="nav-link${
          tabData.current === tab.name ? " active" : ""
        }">${tab.safename}</a></li>`
      })
  ...
  renderExclusions(exclusions) {
    ...
    if (exclusions) {
      ...
      exclusions.available.forEach(category => {
        ...
        categoryList.innerHTML += `<li><a role="button" title="${
          category.description
        }" ${isActive ? `class="active"` : ""} tabindex="0" data-slug="${
          category.slug
        }">#${category.name}</a></li>`
      })
  ...
  renderTopicList(objectList, slugIdentifier, parameters) {
    ...
    if (objectList.length === 0) {
      ...
      objectList.forEach(topic => {
        topics += `<li class="list-group-item"><a href="${slugIdentifier}${
          topic.slug
        }/${params}">${notSafe(topic.title)}<small class="total_entries">${
          topic.count && topic.count !== "0" ? topic.count : ""
        }</small></a></li>`
      })
...
Handler(".exclusion-button", "click", function () {
  this.closest("div")
    .parentNode.querySelector(".exclusion-settings")
    .classList.toggle("dj-hidden")
})
...
Handler("[data-lf-slug]", "click", function () {
  ...
  many(`[data-lf-slug=${this.getAttribute("data-lf-slug")}]`).forEach(el =>
