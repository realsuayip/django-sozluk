  ...
  constructor(button) {
    ...
    this.menuElement.addEventListener("keydown", event => {
      if (
        ...
        this.menuElement.classList.contains("no-arrows")
      ) {
        return
      }
...
Handler("[data-toggle=dropdown]", "keydown", function (event) {
  if (event.key === "ArrowDown") {
    ...
    if (this._dropdownInstance && this._dropdownInstance.popper) {
      const firstFocusableElement = this._dropdownInstance.menuElement.querySelector(
        focusableSelector
      )
...
Handler("[data-toggle=dropdown]", "blur", function (event) {
  ...
  const clickBlur =
    lastClickedElement &&
    lastClickedElement.closest(".dropdown-menu") === dropdown.menuElement
  const focusBlur =
    event.relatedTarget &&
    event.relatedTarget.closest(".dropdown-menu") === dropdown.menuElement
